...
export default function OrcamentoNovo() {
  ...
  const [form, setForm] = useState({ cliente_id: '', validade: '', observacoes: '' })
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <form onSubmit={salvar} className="space-y-4">
        <div className="bg-white rounded-xl p-5 shadow-sm border space-y-3">
          ...
          <input type="datetime-local" placeholder="Validade" value={form.validade}
            onChange={e => setForm({...form,validade:e.target.value})}
            className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
          <textarea placeholder="Observacoes" rows={2} value={form.observacoes}
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border space-y-3">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <input type="number" min="1" value={novoItem.quantidade}
            ...
            <button type="button" onClick={addItem} className="bg-primary text-white px-3 py-2 rounded-lg"><Plus size={16} /></button>
          </div>
          {itens.map((item, i) => (
        ...
        <button type="submit" className="w-full bg-primary text-white py-3 rounded-lg font-semibold hover:bg-primary-dark">
          Salvar Orcamento
        </button>
      </form>
    </div>
