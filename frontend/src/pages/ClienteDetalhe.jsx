...
export default function ClienteDetalhe() {
  ...
  if (!cliente) return <p className="text-gray-400">Carregando...</p>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div><span className="text-gray-500">CPF/CNPJ:</span> <span>{cliente.cpf_cnpj || '-'}</span></div>
          <div><span className="text-gray-500">Telefone:</span> <span>{cliente.telefone || '-'}</span></div>
          <div><span className="text-gray-500">E-mail:</span> <span>{cliente.email || '-'}</span></div>
          <div><span className="text-gray-500">Endereco:</span> <span>{cliente.endereco || '-'}</span></div>
        </div>
      </div>
        ...
        <>
          <div className="bg-white rounded-xl p-6 shadow-sm border">
            ...
            {historico.ordens_servico.length === 0
              ? <p className="text-sm text-gray-400">Nenhuma OS.</p>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border">
            ...
            {historico.orcamentos.length === 0
              ? <p className="text-sm text-gray-400">Nenhum orcamento.</p>
